...
export default function useReveal() {
  useLayoutEffect(() => {
    ...
    initial.forEach(g => {
      ...
      nodes.forEach(n => {
        if (!n.classList.contains('in') && !n.classList.contains('reveal-done'))
          n.classList.add('reveal-prep')
      })
    })
    if (!oVisible && offerings) {
      gather(offerings).forEach(n => {
        if (
          !n.classList.contains('reveal-prep') &&
          !n.classList.contains('in') &&
          !n.classList.contains('reveal-done')
        )
          n.classList.add('reveal-prep')
      })
    }
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => {
          ...
          kids.forEach(k => {
            ...
            if (rect.top < vh2 && rect.bottom > 0) inView.push(k)
            else observe(k)
          })
      ...
      const prev =
        nextIndex === 0
          ? oVisible
            ? offerings
            : hero
          : document.querySelector(order[nextIndex - 1])
      if (!prev) return
