...
export default function ParticleBackground() {
  ...
  const palette = useMemo(() => ['#0ea5e9', '#e0f2fe', '#0284c7', '#d946ef', '#f0abfc'], [])
  ...
  const updateRects = () => {
    ...
    const els = document.querySelectorAll('nav, main, footer, [role="dialog"], button, form')
    const arr = []
    for (let i = 0; i < els.length; i++) {
      const r = els[i].getBoundingClientRect()
      if (r.width && r.height)
        arr.push({ x: r.left - 4, y: r.top - 4, w: r.width + 8, h: r.height + 8 })
    }
